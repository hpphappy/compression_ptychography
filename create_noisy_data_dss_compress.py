...
for n_ph_tx in ['1e4', '4e4', '1e5', '4e5', '1e6', '1.75e6', '4e6', '1e7', '1.75e7', '4e7', '1e8', '1.75e8', '4e8']:
    for postfix in ['', '_ref']:
        ...
        # dest_fname = os.path.join(os.path.dirname(src_fname), os.path.splitext(os.path.basename(src_fname))[0] + '_dss_comp_n{}{}.h5'.format(n_ph_tx, postfix))
        dest_fname = os.path.join(os.path.dirname(src_fname), os.path.splitext(os.path.basename(src_fname))[0] + '_dss_comp_n{}{}.h5'.format(n_ph_tx, postfix))
        ...
        dxchange.write_tiff(abs(n[0]), os.path.join(os.path.dirname(dest_fname), 'dss_comp_n{}{}'.format(n_ph_tx, postfix)), dtype='float32', overwrite=True)
